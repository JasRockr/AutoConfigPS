...
# Validaciones previas a iniciar el pipeline (equivalente al antiguo Script0.ps1),
# convertidas en funcion reutilizable SIN Read-Host: el orquestador decide que hacer
...
function Test-AutoConfigPrerequisites {
    ...
    $checkIndex++
    Write-Progress -Id 1 -Activity 'Validando requisitos' -Status 'Privilegios de Administrador' -PercentComplete (($checkIndex - 1) / $totalChecks * 100)
    ...
    $checkIndex++
    Write-Progress -Id 1 -Activity 'Validando requisitos' -Status 'Version de PowerShell' -PercentComplete (($checkIndex - 1) / $totalChecks * 100)
    ...
    $checkIndex++
    Write-Progress -Id 1 -Activity 'Validando requisitos' -Status 'Adaptador Wi-Fi' -PercentComplete (($checkIndex - 1) / $totalChecks * 100)
    # 3. Adaptador Wi-Fi
    $wifiAvailable = $false
    ...
    $checkIndex++
    Write-Progress -Id 1 -Activity 'Validando requisitos' -Status 'Winget' -PercentComplete (($checkIndex - 1) / $totalChecks * 100)
    # 4. Winget
    ...
    $checkIndex++
    Write-Progress -Id 1 -Activity 'Validando requisitos' -Status 'Archivo config.ps1' -PercentComplete (($checkIndex - 1) / $totalChecks * 100)
    # 5. config.ps1
    ...
    $checkIndex++
    Write-Progress -Id 1 -Activity 'Validando requisitos' -Status 'Credenciales cifradas' -PercentComplete (($checkIndex - 1) / $totalChecks * 100)
    ...
    $checkIndex++
    Write-Progress -Id 1 -Activity 'Validando requisitos' -Status 'Espacio en disco' -PercentComplete (($checkIndex - 1) / $totalChecks * 100)
    # 7. Espacio en disco
    $diskSpaceOk = $false
    ...
    $checkIndex++
    Write-Progress -Id 1 -Activity 'Validando requisitos' -Status 'Conectividad de red' -PercentComplete (($checkIndex - 1) / $totalChecks * 100)
    ...
    Write-Progress -Id 1 -Activity 'Validando requisitos' -Completed
