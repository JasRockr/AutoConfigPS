...
function Initialize-AutoConfigState {
    ...
    if (-not (Test-Path $script:AutoConfigStateDir)) {
        ...
        try {
            icacls $script:AutoConfigStateDir /inheritance:r /grant "BUILTIN\Administrators:(OI)(CI)F" /grant "SYSTEM:(OI)(CI)F" | Out-Null
        } catch {
...
function Get-AutoConfigStatusSummary {
    ...
    switch ($State.Status) {
        'Completed' {
            return [PSCustomObject]@{ Status = 'Completed'; Text = 'AutoConfigPS: configuracion completada.' }
        }
        'Failed' {
...
function Save-AutoConfigStatusMarker {
    ...
    try {
        ...
        icacls $script:AutoConfigStatusMarkerPath /grant "BUILTIN\Users:(R)" | Out-Null
    } catch {
