    ...
    foreach ($logFile in @($script:AutoConfigSuccessLog, $script:AutoConfigErrorLog)) {
        if (-not (Test-Path $logFile)) {
            ...
            try {
                ...
                # espanol (grupo "Administradores") icacls no resuelve el nombre
                # en ingles y falla, dejando el log sin el permiso esperado -
                # encontrado en pruebas reales.
                # *S-1-5-32-545:(R) = BUILTIN\Users con solo LECTURA: el escribir el
                # log queda restringido a Administrators/SYSTEM, pero cualquier
                # usuario (incluido el tecnico presente) puede ABRIR los logs para
                # revisar el avance/errores del proceso, sin poder modificarlos.
                icacls $logFile /inheritance:r /grant "*S-1-5-32-544:(F)" /grant "*S-1-5-18:(F)" /grant "*S-1-5-32-545:(R)" | Out-Null
            } catch {
            ...
            # El log ya existe (re-ejecucion, o creado por una version anterior sin el
            # permiso de lectura): asegurar de todos modos el acceso de lectura para
            # Users, de forma idempotente. icacls /grant es aditivo, no reemplaza el
            # resto del ACL.
            try {
                icacls $logFile /grant "*S-1-5-32-545:(R)" | Out-Null
            } catch {
                # No bloqueante (ver arriba).
            }
        }
    }
}
